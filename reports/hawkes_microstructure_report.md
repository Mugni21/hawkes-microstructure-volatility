...
4. Hawkes contrarian scheduling using buy and sell intensities as adverse-selection pressure to fade.
5. Hawkes momentum scheduling using buy and sell intensities as short-term urgency pressure to follow.

The two Hawkes schedules represent competing interpretations of the same reduced-form signal. For a buy parent order, the contrarian rule executes less when buy pressure is high and more when sell pressure is high, while the momentum rule does the reverse. For sell parent orders, the rules are symmetric. Performance is summarized with average execution price and implementation shortfall relative to the arrival price. The simulator evaluates which interpretation performs better in a specific historical window and does not claim either is universally correct.
...
- Hawkes intensities are used only as reduced-form order-flow pressure signals under contrarian and momentum interpretations.
